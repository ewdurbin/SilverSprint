...
{	
	...
	// We keep the original options so that we can still regex against them, but change up what is displayed
	setOptions(rawOpts);
	mOptionsDisplay = displayOpts;
}
...
void DropDown::onMouseMove(ci::app::MouseEvent event){
	...
	// mHighlight contains which index the mouse is hovered over
	if(bOpen){
		vec2 pos = Model::instance().localToGlobal(event.getPos());
...
void DropDown::setSelected(const int &num) {
	...
	CI_LOG_I("DropDown OptionSelect: ") << num;
	
	mSelectedStr = mOptions[num];
}
...
void DropDown::draw() {
	...
	if(!bOpen){
		...
		if(mOptions.size() > 0){
			gl::ScopedColor scC(Color::black());
			mFont->drawString(
				mOptionsDisplay[mSelectedIndex],
				Rectf(mBgRect.getUpperLeft() + vec2(25, 25), mBgRect.getLowerRight() ), 
				vec2(0),
		...
		gl::ScopedColor scC(Color::black());
		for( int i=0; i< mOptionsDisplay.size(); i++){
			if(mHighlight == i){
				gl::ScopedColor scBg(Color::white());
				gl::drawSolidRect(Rectf(x1, y1 + getHeight() * i, x2, y2 + (getHeight()+1) * i));
			}
			
			mFont->drawString(mOptionsDisplay[i], (Rectf)*this + vec2(25, 25 + i * getHeight()), vec2(0),
				Model::instance().getTfDrawOpts());
			
			float yp = y2 + getHeight() * i + 1;
			
			if(i != mOptionsDisplay.size()-1){
